...
// Initial Seed Data (Empty by default - awaiting in-code contracted doctors injection)
...
function loadPersistentData() {
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      // Clean legacy mock IDs (doc-1 to doc-10, etc.)
      const isLegacyMockDoc = (d: any) => {
        if (!d || !d.id) return false;
        const num = parseInt(d.id.replace('doc-', ''), 10);
        return !isNaN(num) && num <= 20;
      };
      doctorsStore = (parsed.doctors || []).filter((d: any) => !isLegacyMockDoc(d));
      bookingsStore = (parsed.bookings || []).filter((b: any) => !b.id?.startsWith('BK-2026-'));
