  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 font-['Tajawal',sans-serif]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-200 dark:border-slate-800">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-[#002b49] dark:text-white">الملف الطبي الرقمي</h1>
        ...
        <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
          ...
          <button
            ...
            className="bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200/80 dark:border-emerald-800/80 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 text-emerald-700 dark:text-emerald-300 text-xs sm:text-sm font-bold py-2.5 px-3.5 rounded-xl flex items-center gap-1.5 cursor-pointer shadow-xs transition-all"
            title="مسح وتصوير الروشتة الورقية وتحويلها تلقائياً لبيانات رقمية"
          >
            <Sparkles className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
            <span>ماسح الروشتات (OCR)</span>
          </button>
          ...
          <button
            ...
            className="bg-purple-50 dark:bg-purple-950/60 border border-purple-200/80 dark:border-purple-800/80 hover:bg-purple-100 dark:hover:bg-purple-900/60 text-purple-700 dark:text-purple-300 text-xs sm:text-sm font-bold py-2.5 px-3.5 rounded-xl flex items-center gap-1.5 cursor-pointer shadow-xs transition-all"
          ...
          <button
            ...
            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs sm:text-sm font-bold py-2.5 px-3.5 rounded-xl flex items-center gap-1.5 cursor-pointer shadow-xs transition-all"
            title="مشاركة الملف مع طبيب العيادة عبر كود مؤقت"
          >
            <QrCode className="w-4 h-4 text-[#0070cd] dark:text-blue-400" />
            <span>مشاركة الملف مع الطبيب</span>
          </button>
          ...
          <button
            ...
            className="bg-blue-50 dark:bg-slate-800 border border-blue-200/80 dark:border-slate-700 hover:bg-blue-100 dark:hover:bg-slate-700 text-[#0070cd] dark:text-blue-400 text-xs sm:text-sm font-bold py-2.5 px-3.5 rounded-xl flex items-center gap-1.5 cursor-pointer shadow-xs transition-all"
            title="فحص أمان وتعارضات الأدوية"
          >
            <Pill className="w-4 h-4" />
            <span>فاحص تعارض الأدوية</span>
          </button>

          <button
            onClick={() => setShowAddModal(true)}
            className="bg-[#0070cd] hover:bg-[#005bb0] text-white text-xs sm:text-sm font-bold py-2.5 px-4 rounded-xl flex items-center gap-1.5 cursor-pointer shadow-sm transition-all"
          >
      ...
      {detectedInteractions.length > 0 && (
        <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/60 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-in fade-in">
          ...
          <button
            type="button"
            onClick={() => setShowDrugCheckerModal(true)}
            className="bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs px-3.5 py-2 rounded-xl shrink-0 transition-colors cursor-pointer"
          >
            عرض تفاصيل التعارض
          </button>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredRecords.map((record) => (
            <div
              key={record.id}
              className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-xs hover:border-blue-300 dark:hover:border-slate-700 transition-all flex flex-col justify-between"
            >
              <div>
                ...
                <h3 className="font-black text-slate-900 dark:text-white text-base mb-1">{record.title}</h3>
                <div className="text-xs text-slate-600 dark:text-slate-400 space-y-0.5 mb-3">
                ...
                {record.notes && (
                  <p className="text-xs text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/60 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800 mb-3 leading-relaxed">
                    {record.notes}
                  </p>
                )}
                ...
                {record.medications && record.medications.length > 0 && (
                  <div className="space-y-1.5 mb-3">
                    <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 block">الأدوية الموصوفة:</span>
                    {record.medications.map((med, idx) => (
                      <div key={idx} className="bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 p-2 rounded-lg text-xs flex items-center justify-between">
                        <div className="flex items-center gap-1.5">
                          <Pill className="w-3.5 h-3.5 text-[#0070cd] dark:text-blue-400" />
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 dark:bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl p-6 shadow-2xl border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-100 dark:border-slate-800">
              <h3 className="font-black text-slate-900 dark:text-white text-base">إضافة مستند أو روشتة طبية جديدة</h3>
              <button onClick={() => setShowAddModal(false)} className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">
                <X className="w-5 h-5" />
              </button>
            </div>

            <form onSubmit={handleSaveNewRecord} className="space-y-3 text-xs">
              <div>
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">عنوان التقرير أو التشخيص *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-2.5 text-xs text-slate-900 dark:text-slate-100 outline-none focus:border-[#0070cd]"
                />
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">اسم الطبيب أو المعمل</label>
                  <input
                    type="text"
                    placeholder="د. أحمد..."
                    value={newDoctor}
                ...
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">تاريخ الكشف</label>
                  <input
                    type="date"
                    value={newDate}
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">اسم الدواء الموصوف (إن وجد)</label>
                <input
                  type="text"
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold transition-all duration-200 cursor-pointer ${
                      isListening
                        ? 'bg-red-500 text-white animate-pulse shadow-xs shadow-red-500/30 ring-2 ring-red-300 dark:ring-red-900'
                        : 'bg-blue-50 dark:bg-slate-800 text-[#0070cd] dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-slate-700 border border-blue-200/60 dark:border-slate-700'
                    }`}
                ...
                <textarea
                  ...
                  className={`w-full bg-slate-50 dark:bg-slate-800 border rounded-xl p-2.5 text-xs text-slate-900 dark:text-slate-100 outline-none transition-all ${
                    isListening 
                      ...
                      : 'border-slate-200 dark:border-slate-700 focus:border-[#0070cd]'
                  }`}
                />
              </div>
              ...
              <div className="pt-2 flex gap-2">
                <button
                  type="submit"
                  className="flex-1 bg-[#0070cd] text-white font-bold py-2.5 rounded-xl hover:bg-[#005bb0]"
                >
                  حفظ في الملف الطبي
                </button>
                <button
                  ...
                  className="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold py-2.5 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700"
                >
                  إلغاء
                </button>
              </div>
      ...
      <DrugInteractionCheckerModal
        ...
        initialMedicines={allActiveMedNames.length > 0 ? allActiveMedNames : ['أسبرين', 'بروفين']}
      />
