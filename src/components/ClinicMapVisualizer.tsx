...
export const ClinicMapVisualizer: React.FC<ClinicMapVisualizerProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-[0_4px_20px_rgba(0,43,73,0.06)] overflow-hidden font-['Tajawal',sans-serif] transition-colors duration-200">
      ...
      <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-950/60 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="inline-flex items-center gap-1.5 bg-blue-50 dark:bg-blue-950/70 text-[#0066b2] dark:text-blue-400 text-xs font-bold px-3 py-1 rounded-full border border-blue-200/80 dark:border-blue-900/60">
              <Compass className="w-3.5 h-3.5 text-[#0066b2] dark:text-blue-400" />
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          <select
            ...
            className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold py-2 px-3 rounded-xl outline-none focus:border-[#0066b2] cursor-pointer"
          >
          ...
          <div className="flex items-center bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 p-0.5 rounded-xl">
            <button
              type="button"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[460px]">
        ...
        <div className="lg:col-span-8 relative bg-slate-100 dark:bg-slate-950 overflow-hidden flex items-center justify-center p-4 select-none min-h-[360px] sm:min-h-[460px]">
          ...
          <div 
            className={`absolute inset-0 transition-opacity duration-500 ${
              mapLayer === 'satellite' 
                ? 'bg-gradient-to-br from-slate-900 via-slate-950 to-blue-950 opacity-95' 
                : 'bg-gradient-to-br from-[#e8f2fa] via-[#f1f7fc] to-[#e4eef6] dark:from-slate-950 dark:via-slate-900 dark:to-slate-950'
            }`}
          >
            ...
            <svg className="w-full h-full opacity-30 dark:opacity-20 pointer-events-none" xmlns="http://www.w3.org/2000/svg">
              <defs>
                <pattern id="grid-pattern" width="40" height="40" patternUnits="userSpaceOnUse">
                  <path d="M 40 0 L 0 0 0 40" fill="none" stroke="currentColor" strokeWidth="0.5" className="text-blue-400 dark:text-slate-700" />
                </pattern>
              </defs>
          ...
          <div className="absolute top-4 left-6 text-[10px] sm:text-xs font-bold text-slate-400 dark:text-slate-500 pointer-events-none uppercase tracking-wider flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></span>
            <span>البحر الأبيض المتوسط • الإسكندرية والساحل</span>
          </div>

          <div className="absolute bottom-4 right-6 text-[10px] sm:text-xs font-bold text-slate-400 dark:text-slate-500 pointer-events-none uppercase tracking-wider">
            ...
            {displayedAreas.map((geo, idx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {/* Pin Bubble */}
                    <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold shadow-lg transition-all ${
                      isSelected
                        ? 'bg-[#0066b2] text-white ring-4 ring-blue-300 dark:ring-blue-900/80 shadow-blue-500/40'
                        : count > 0
                        ? 'bg-emerald-600 text-white hover:bg-emerald-700'
                        : 'bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-700 hover:border-[#0066b2]'
                    }`}>
                      <MapPin className={`w-3.5 h-3.5 shrink-0 ${isSelected ? 'text-white' : count > 0 ? 'text-white' : 'text-[#0066b2] dark:text-blue-400'}`} />
          ...
          <div className="absolute bottom-4 left-4 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md p-2.5 rounded-2xl border border-slate-200 dark:border-slate-800 text-[11px] space-y-1 shadow-sm">
        ...
        <div className="lg:col-span-4 p-5 sm:p-6 bg-white dark:bg-slate-900 border-t lg:border-t-0 lg:border-r border-slate-200 dark:border-slate-800 flex flex-col justify-between">
          <div>
            {activePin ? (
              <div>
                <div className="flex items-start justify-between gap-2 pb-3 border-b border-slate-100 dark:border-slate-800 mb-4">
                  <div>
                    <span className="text-[11px] font-bold bg-blue-50 dark:bg-blue-950 text-[#0066b2] dark:text-blue-400 px-2 py-0.5 rounded-md border border-blue-100 dark:border-blue-900/60">
                      {activePin.city}
                    </span>
                    <h3 className="text-lg font-black text-[#002b49] dark:text-white mt-1">
                      حي {activePin.area}
                    </h3>
                  </div>
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl text-[#0066b2] dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-slate-800 transition-colors cursor-pointer flex items-center gap-1 text-xs font-bold"
                ...
                <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-100 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-300 mb-4">
                  <span className="font-bold text-slate-800 dark:text-slate-200 block mb-0.5">أبرز الشوارع والمراكز الطبية:</span>
                ...
                <div>
                  <div className="flex items-center justify-between text-xs font-bold text-slate-700 dark:text-slate-300 mb-2.5">
                    ...
                    <span className="text-[#0066b2] dark:text-blue-400">({activePinDoctors.length} متاح)</span>
                  </div>

                  {activePinDoctors.length === 0 ? (
                    <div className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-200/80 dark:border-slate-800 text-center">
                      <Building2 className="w-8 h-8 text-slate-400 mx-auto mb-2" />
                      <p className="text-xs font-bold text-slate-700 dark:text-slate-300">لم يتم تسجيل عيادات في هذه المنطقة بعد</p>
                      ...
                      {onOpenJoinDoctor && (
                        <button
                          ...
                          className="mt-3 text-xs text-[#0066b2] dark:text-blue-400 font-bold hover:underline cursor-pointer"
                        >
                    ...
                    <div className="space-y-2.5 max-h-60 overflow-y-auto pr-0.5">
                      {activePinDoctors.map((doc) => (
                        <div
                          key={doc.id}
                          className="p-3 bg-slate-50 dark:bg-slate-800/60 hover:bg-blue-50/40 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-2xl transition-all"
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <h4 
                                onClick={() => onOpenProfile(doc)}
                                className="text-xs font-black text-slate-900 dark:text-white hover:text-[#0066b2] cursor-pointer"
                              >
                                {doc.name}
                              </h4>
                              <p className="text-[11px] text-slate-600 dark:text-slate-400">{doc.specialty}</p>
                            </div>
                            <span className="text-xs font-black text-[#0066b2] dark:text-blue-400 shrink-0">
                              {doc.fee} ج.م
                            </span>
                          </div>
                          ...
                          <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-200/60 dark:border-slate-700/60 text-[11px]">
                            <span className="text-slate-500 dark:text-slate-400 truncate max-w-[140px]">{doc.landmark}</span>
                            <button
                              type="button"
                              onClick={() => onOpenProfile(doc)}
                              className="text-[#0066b2] dark:text-blue-400 font-bold hover:underline cursor-pointer"
                            >
                              حجز موعد
                            </button>
                          </div>
              ...
              <div className="py-8 text-center">
                <div className="w-14 h-14 rounded-2xl bg-blue-50 dark:bg-slate-800 text-[#0066b2] dark:text-blue-400 flex items-center justify-center mx-auto mb-3 border border-blue-100 dark:border-slate-700">
                ...
                <button
                  ...
                  className={`text-[11px] px-2.5 py-1 rounded-lg border transition-all cursor-pointer ${
                    activePin?.area === geo.area
                      ...
                      : 'bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:border-[#0066b2]'
                  }`}
                >
                  {geo.area}
                </button>
