...
export const AccessibilityBar: React.FC<AccessibilityBarProps> = ({
  ...
  const handleToggleSpeech = () => {
    ...
    if (isSpeaking) {
      ...
      window.speechSynthesis.cancel();
      const textToRead = "أهلاً بك في منصة دكتورنا الطبية. يمكنك البحث عن أفضل الأطباء والعيادات في محافظات مصر، حجز موعدك فوراً، أو طلب استشارة بالفيديو دون انتظار. لأي مساعدة يمكنك الضغط على الخط الساخن 16676 أو الطوارئ 123.";
  ...
  return (
    <div className="relative z-30 font-['Tajawal',sans-serif]">
      {/* Accessibility Floating/Embedded Pill */}
      <div className="flex items-center gap-1.5 bg-slate-100/90 dark:bg-slate-800/90 backdrop-blur-md px-2.5 py-1 rounded-full border border-slate-200/80 dark:border-slate-700/80 shadow-xs">
        <button
          type="button"
          onClick={() => setIsOpen(!isOpen)}
          className={`flex items-center gap-1.5 text-xs font-bold px-2 py-0.5 rounded-full transition-all cursor-pointer ${
            isOpen || preferences.highContrast || preferences.textScale !== 'normal'
              ? 'bg-[#0071e3] text-white shadow-xs'
              : 'text-slate-700 dark:text-slate-300 hover:text-[#0071e3] dark:hover:text-sky-400'
          }`}
        ...
        <div className="flex items-center gap-0.5 bg-white dark:bg-slate-900 rounded-full p-0.5 border border-slate-200/60 dark:border-slate-700/60">
          <button
            type="button"
          ...
          <button
            ...
            className={`px-1.5 py-0.5 text-[11px] font-extrabold rounded-full transition-colors cursor-pointer ${
      ...
      {isOpen && (
        <div className="absolute top-full mt-2 left-0 w-72 apple-card p-4 rounded-2xl shadow-xl z-50 border border-slate-200 dark:border-slate-700 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-800/70 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="font-bold text-slate-800 dark:text-slate-100">تباين عالي (وضوح فائق)</div>
                  <div className="text-[10px] text-slate-500 dark:text-slate-400">تغميق النصوص وزيادة وضوح الحواف</div>
                </div>
              </div>
              <button
                type="button"
                ...
                className={`w-10 h-6 flex items-center rounded-full p-1 transition-colors cursor-pointer ${
                  preferences.highContrast ? 'bg-[#0071e3] justify-end' : 'bg-slate-300 dark:bg-slate-600 justify-start'
                }`}
              >
            ...
            <div className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-800/70 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="font-bold text-slate-800 dark:text-slate-100">المساعد الصوتي الناطق</div>
                  <div className="text-[10px] text-slate-500 dark:text-slate-400">قراءة التعليمات والإرشادات بصوت واضح</div>
                </div>
              </div>
              <button
                type="button"
