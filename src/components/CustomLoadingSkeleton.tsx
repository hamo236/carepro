...
export const CustomLoadingSkeleton: React.FC<LoadingSkeletonProps> = ({
  ...
  if (type === 'doctor-card') {
    return (
      <div className="space-y-4 w-full animate-pulse font-['Tajawal',sans-serif]">
        {items.map((i) => (
          <div
            key={i}
            className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-5 sm:p-6 flex flex-col lg:flex-row gap-6 shadow-xs"
          >
            {/* Info Side */}
            <div className="flex-1 space-y-4">
              ...
              {/* Details Box */}
              <div className="h-16 bg-slate-100 dark:bg-slate-800/70 rounded-2xl" />
            </div>

            {/* Schedule Side */}
            <div className="w-full lg:w-80 bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3">
  ...
  if (type === 'record-card') {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full animate-pulse font-['Tajawal',sans-serif]">
        {items.map((i) => (
          <div
            key={i}
