...
export const NotificationsModal: React.FC<NotificationsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 dark:bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200 font-['Tajawal',sans-serif]">
      <div 
      ...
      >
        ...
        <div className="bg-[#002b49] dark:bg-slate-950 text-white p-5 flex items-center justify-between border-b border-slate-800">
          <div className="flex items-center gap-2">
            <Bell className="w-5 h-5 text-[#0070cd] dark:text-blue-400" />
            <h3 className="font-black text-sm">التنبيهات ومحرك التذكيرات الذكية</h3>
          </div>
        ...
        {/* Tab Selector */}
        <div className="flex items-center border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-1.5">
          <button
            type="button"
            ...
            className={`flex-1 text-xs font-bold py-2 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
              activeTab === 'alerts'
                ? 'bg-white dark:bg-slate-800 text-[#0070cd] dark:text-blue-400 shadow-xs'
              ...
              <>
                <div className="flex items-center justify-between pb-1">
                  ...
                  <button
                    ...
                    className="text-[11px] font-bold text-[#0070cd] dark:text-blue-400 hover:underline cursor-pointer"
                  >
                    تحديد الكل كمقروء
                  </button>
                </div>
                ...
                {providerNotice && (
                  <div className="p-3.5 bg-amber-50/90 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/50 rounded-2xl animate-in fade-in">
                ...
                {activeBooking && (
                  <div className="p-3.5 bg-blue-50/80 dark:bg-slate-800/80 border border-blue-200/80 dark:border-slate-700 rounded-2xl">
                    <div className="flex items-center justify-between text-xs mb-1">
                      <span className="font-bold text-[#0070cd] dark:text-blue-400 flex items-center gap-1">
                        <Calendar className="w-3.5 h-3.5" />
                        <span>تذكير بموعد العيادة القادم</span>
                      </span>
                      <span className="text-[10px] bg-blue-100 dark:bg-slate-700 text-[#0070cd] dark:text-blue-300 px-2 py-0.5 rounded-full font-bold">اليوم</span>
                    </div>
                    <p className="text-xs text-slate-800 dark:text-slate-200 leading-relaxed">
                      موعدك مع <strong>{activeBooking.doctorName}</strong> مجدول لـ ({activeBooking.day} - {activeBooking.slot}) برقم حجز <strong className="font-mono text-[#0070cd] dark:text-blue-400">{activeBooking.id}</strong>.
                    </p>
                  </div>
                )}
        ...
        {activeTab === 'settings' && (
          <div className="p-5 space-y-4 max-h-96 overflow-y-auto">
            ...
            <div className="p-4 bg-blue-50/70 dark:bg-slate-800/70 rounded-2xl border border-blue-200 dark:border-slate-700 flex items-center justify-between gap-3">
              <div>
              ...
              {permGranted ? (
                <span className="text-emerald-700 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-950/60 text-[11px] font-black px-3 py-1.5 rounded-xl flex items-center gap-1 shrink-0">
                ...
                <button
                  ...
                  className="bg-[#0070cd] hover:bg-[#005bb0] text-white text-xs font-bold px-3 py-1.5 rounded-xl transition-colors shrink-0 cursor-pointer"
                >
                  تفعيل الإذن
                </button>
              )}
            ...
            <div className="space-y-2.5">
              <label className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer">
                ...
                <input
                  ...
                  className="w-4 h-4 text-[#0070cd] rounded focus:ring-blue-500"
                />
              </label>
            ...
            <div className="pt-2 flex justify-end">
              <button
                ...
                className="text-xs font-bold text-[#0070cd] dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-slate-800 px-3.5 py-2 rounded-xl transition-colors flex items-center gap-1.5 cursor-pointer border border-blue-200/80 dark:border-slate-700"
              >
        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-200 dark:border-slate-700 text-center">
          <button
            onClick={onClose}
            className="w-full bg-[#0070cd] hover:bg-[#005bb0] text-white text-xs font-bold py-2.5 rounded-xl transition-colors cursor-pointer"
          >
            إغلاق
          </button>
        </div>
