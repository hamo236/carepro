...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-['Tajawal',sans-serif]">
      ...
      {/* Admin Top Header */}
      <div className="bg-[#002b49] text-white rounded-3xl p-6 sm:p-8 shadow-sm mb-8 relative overflow-hidden">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs bg-blue-500/30 text-blue-200 font-bold px-2.5 py-0.5 rounded-full border border-blue-400/30 flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5" />
                <span>إدارة المنصة والاعتماد الطبي</span>
              </span>
              <span className="text-xs text-slate-300">نظام الرقابة والحوكمة الطبية</span>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 self-start lg:self-auto no-print">
            <button
              ...
              className="bg-white/15 hover:bg-white/25 text-white border border-white/20 px-4 py-3 rounded-2xl text-xs sm:text-sm font-bold flex items-center gap-2 transition-all cursor-pointer shadow-xs active:scale-98"
            ...
            <button
              ...
              className="bg-[#0070cd] hover:bg-[#005bb0] text-white px-5 py-3 rounded-2xl text-xs sm:text-sm font-bold flex items-center gap-2 shadow-lg shadow-blue-900/40 cursor-pointer active:scale-98"
            >
        ...
        {/* Top KPIs Banner */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8 pt-6 border-t border-slate-700/60 printable-card">
          <div className="bg-white/10 backdrop-blur-xs p-3.5 rounded-2xl border border-white/10">
            ...
            <span className="text-2xl font-black text-amber-300 font-mono">{pendingRequests.length}</span>
          </div>

          <div className="bg-white/10 backdrop-blur-xs p-3.5 rounded-2xl border border-white/10">
            <span className="text-xs text-slate-300 font-bold block mb-1">الأطباء والعيادات المعتمدة</span>
            <span className="text-2xl font-black text-emerald-300 font-mono">{doctors.length}</span>
          </div>

          <div className="bg-white/10 backdrop-blur-xs p-3.5 rounded-2xl border border-white/10">
            <span className="text-xs text-slate-300 font-bold block mb-1">إجمالي الحجوزات المنفذة</span>
            <span className="text-2xl font-black text-white font-mono">{bookings.length}</span>
          </div>

          <div className="bg-white/10 backdrop-blur-xs p-3.5 rounded-2xl border border-white/10">
            <span className="text-xs text-slate-300 font-bold block mb-1">نسبة التوثيق والاعتماد</span>
            <span className="text-xl font-black text-white font-mono">100% معتمد</span>
          </div>
        </div>
      </div>

      {/* Navigation Tabs */}
      <div className="flex flex-wrap items-center gap-2 mb-6 border-b border-slate-200 dark:border-slate-800 pb-3 no-print">
        <button
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-6">
            <div className="bg-[#002b49] text-white p-5 flex items-center justify-between">
              <h3 className="font-black text-base">إضافة وتفعيل عيادة أو طبيب جديد</h3>
            ...
            <form onSubmit={handleCreateDoctor} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">اسم الطبيب أو المركز الطبي</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">التخصص الطبي</label>
                  <select
                    value={newDocSpecialtyId}
                    onChange={(e) => setNewDocSpecialtyId(e.target.value)}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-xs font-bold text-slate-900 dark:text-white outline-none cursor-pointer"
                  >
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">المحافظة</label>
                  <input
                    type="text"
                    value={newDocCity}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">المنطقة</label>
                  <input
                    type="text"
                    value={newDocArea}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">رقم الهاتف للتواصل</label>
                  <input
                    type="tel"
                    placeholder="01012345678"
                    value={newDocPhone}
                    onChange={(e) => setNewDocPhone(e.target.value)}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-xs text-slate-900 dark:text-white outline-none font-mono"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  className="bg-[#0070cd] hover:bg-[#005bb0] text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-xs cursor-pointer"
                >
                  حفظ وتفعيل الطبيب
                </button>
              </div>
