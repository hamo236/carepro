...
export const DrugInteractionCheckerModal: React.FC<DrugInteractionCheckerModalProps> = ({
  ...
  const [selectedMeds, setSelectedMeds] = useState<string[]>(
    initialMedicines.length > 0 ? initialMedicines : ['أسبرين', 'بروفين']
  );
...
${generalPrecautions ? `إرشادات عامة: ${generalPrecautions}` : ''}`;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 dark:bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto animate-in fade-in font-['Tajawal',sans-serif]">
      <div 
        className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-6 flex flex-col max-h-[90vh] printable-medical-report"
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black text-white flex items-center gap-2">
                ...
                <span className="bg-indigo-500/20 text-indigo-300 text-[10px] font-bold px-2 py-0.5 rounded-full border border-indigo-500/30 no-print">
                  مدعوم بـ Gemini AI
                </span>
              </h2>
              <p className="text-xs text-blue-200">فحص أمان وسلامة الأدوية المتزامنة لتجنب التفاعلات الضارة والأعراض الجانبية</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-sm text-slate-700 dark:text-slate-300">
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 dark:text-slate-100 outline-none focus:border-[#0070cd]"
              />
              <button
                type="button"
                onClick={() => handleAddMed(customInput)}
                className="bg-[#0070cd] hover:bg-[#005bb0] text-white text-xs font-bold px-4 py-2.5 rounded-xl transition-colors flex items-center gap-1 cursor-pointer"
              >
            ...
            {/* Quick chips */}
            <div>
              <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 block mb-1.5">أدوية شائعة للفحص السريع:</span>
              <div className="flex flex-wrap gap-1.5">
                {COMMON_DRUG_DATABASE.slice(0, 8).map((med, i) => (
                  <button
                    ...
                    className="text-[11px] bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer"
                  >
                    + {med}
                  </button>
                ))}
            ...
            {selectedMeds.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {selectedMeds.map((med, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center gap-1.5 bg-blue-50 dark:bg-slate-800 text-[#0070cd] dark:text-blue-400 border border-blue-200/80 dark:border-slate-700 px-3 py-1.5 rounded-xl text-xs font-bold"
                  >
                    <span>{med}</span>
                    <button
                      type="button"
          ...
          <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
              ...
              <div className="flex items-center gap-2">
                {isAiChecking ? (
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-[#0070cd] flex items-center gap-1 cursor-pointer no-print"
                  >
            ...
            {selectedMeds.length < 2 ? (
              <div className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400 flex items-center gap-2">
                <Info className="w-4 h-4 text-[#0070cd] shrink-0" />
              ...
              <div className="p-8 text-center bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
                <Loader2 className="w-8 h-8 text-[#0070cd] animate-spin mx-auto mb-2" />
              ...
              <div className="space-y-4 animate-in fade-in">
                ...
                <div className={`p-4 rounded-2xl border printable-card ${
                  hasInteractions
                ...
                {aiAlertsList.length > 0 && (
                  <div className="space-y-3">
                    {aiAlertsList.map((inter: any, idx: number) => (
                      <div
                        key={idx}
                        className={`p-4 rounded-2xl border printable-card ${
                      ...
                      >
                        ...
                        <div className="text-xs text-slate-700 dark:text-slate-300 bg-white/80 dark:bg-slate-900/80 p-2.5 rounded-xl border border-slate-200/60 dark:border-slate-800">
                          <strong className="text-[#0070cd] dark:text-blue-400">التوصية السريرية:</strong> {inter.clinicalAdvice}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {(generalPrecautions || foodInteractionsList.length > 0) && (
                  <div className="p-3.5 bg-amber-50/70 dark:bg-slate-850 rounded-2xl border border-amber-200/80 dark:border-slate-750 text-xs text-slate-700 dark:text-slate-300 space-y-1.5 printable-card">
                    <div className="flex items-center gap-1.5 font-bold text-amber-900 dark:text-amber-300">
                      <Apple className="w-4 h-4 text-amber-600" />
                      <span>إرشادات التناول مع الأغذية والسوائل:</span>
                    </div>
                    {generalPrecautions && (
                      <p className="text-xs leading-relaxed pr-1">{generalPrecautions}</p>
                    )}
                    {foodInteractionsList.length > 0 && (
                      <ul className="list-disc list-inside space-y-1 pr-1 text-[11px] mt-1">
              ...
              <div className="p-4 bg-emerald-50 dark:bg-emerald-950/40 rounded-2xl border border-emerald-200 dark:border-emerald-900/50 flex items-start gap-3 printable-card">
              ...
              <div className="space-y-3">
                {localAlerts.map((alert, idx) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-slate-700 dark:text-slate-300 bg-white/70 dark:bg-slate-900/70 p-2.5 rounded-xl border border-slate-200/60 dark:border-slate-800">
                      <strong className="text-[#0070cd] dark:text-blue-400">التوصية الطبية:</strong> {alert.clinicalAdvice}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* Footer */}
        <div className="bg-slate-50 dark:bg-slate-950 p-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs no-print">
          ...
          <button
            type="button"
            onClick={onClose}
            className="bg-[#0070cd] hover:bg-[#005bb0] text-white font-bold px-5 py-2 rounded-xl transition-colors cursor-pointer"
          >
            إغلاق الفاحص
          </button>
        </div>
