...
export const DoctorDashboard: React.FC<DoctorDashboardProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-['Tajawal',sans-serif]">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xs mb-8">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-blue-50 dark:bg-slate-800 text-[#0070cd] dark:text-blue-400 border border-blue-200 dark:border-slate-700 flex items-center justify-center shrink-0">
              <Stethoscope className="w-8 h-8" />
            </div>
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 font-bold px-2.5 py-0.5 rounded-full border border-emerald-300 dark:border-emerald-800">
                  لوحة تحكم الطبيب والعيادة
                </span>
                <span className="text-xs text-slate-500 font-bold">حساب العيادة المعتمد</span>
              </div>
              <h1 className="text-xl sm:text-2xl font-black text-[#002b49] dark:text-white">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className="px-4 py-2.5 bg-gradient-to-r from-teal-700 to-emerald-700 hover:from-teal-800 hover:to-emerald-800 text-white rounded-2xl text-xs font-bold flex items-center gap-2 shadow-sm transition-all cursor-pointer"
            >
              <Activity className="w-4 h-4 text-emerald-300 animate-pulse" />
              <span>غرفة عمليات المواعيد ومنع التأخير</span>
            </button>

            <div className="bg-slate-50 dark:bg-slate-800 p-2 rounded-2xl border border-slate-200 dark:border-slate-700 flex items-center gap-2">
              <span className="text-xs font-bold text-slate-600 dark:text-slate-300">العيادة الحالية:</span>
              <select
                value={selectedDoctorId}
                onChange={(e) => setSelectedDoctorId(e.target.value)}
                className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 text-xs font-bold py-1.5 px-3 rounded-xl border border-slate-200 dark:border-slate-700 outline-none cursor-pointer"
              >
                {doctors.map(doc => (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-slate-100 dark:border-slate-800">
          <div className="bg-blue-50/60 dark:bg-slate-800/60 p-3.5 rounded-2xl border border-blue-100 dark:border-slate-700">
            <span className="text-xs text-slate-500 dark:text-slate-400 font-bold block mb-1">المرضى في الانتظار</span>
            <span className="text-2xl font-black text-[#0070cd] dark:text-blue-400 font-mono">{waitingPatients.length}</span>
          </div>

          <div className="bg-amber-50/60 dark:bg-slate-800/60 p-3.5 rounded-2xl border border-amber-100 dark:border-slate-700">
            <span className="text-xs text-slate-500 dark:text-slate-400 font-bold block mb-1">في غرفة الكشف</span>
            <span className="text-2xl font-black text-amber-600 dark:text-amber-400 font-mono">
              {inConsultationPatient ? '١ مريض' : 'لا يوجد'}
            </span>
          </div>

          <div className="bg-emerald-50/60 dark:bg-slate-800/60 p-3.5 rounded-2xl border border-emerald-100 dark:border-slate-700">
            <span className="text-xs text-slate-500 dark:text-slate-400 font-bold block mb-1">تم الكشف اليوم</span>
            <span className="text-2xl font-black text-emerald-600 dark:text-emerald-400 font-mono">{completedPatients.length}</span>
          </div>

          <div className="bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-700">
            <span className="text-xs text-slate-500 dark:text-slate-400 font-bold block mb-1">متوسط مدة الانتظار</span>
            <span className="text-xl font-black text-slate-900 dark:text-white">{currentDoctor?.waitingTime || '٢٠ دقيقة'}</span>
          </div>
        </div>
      </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="bg-[#0070cd] hover:bg-[#005bb0] active:scale-98 text-white px-5 py-2.5 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-md shadow-blue-500/20 cursor-pointer"
                  >
                    <FileText className="w-4 h-4" />
                  ...
                  <button
                    ...
                    className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-1 cursor-pointer"
                  >
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-xs">
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100 dark:border-slate-800">
              <h3 className="font-black text-base text-slate-900 dark:text-white flex items-center gap-2">
                <Users className="w-4 h-4 text-[#0070cd] dark:text-blue-400" />
                <span>قائمة انتظار المرضى بالعيادة ({waitingPatients.length})</span>
              </h3>
              <span className="text-xs text-slate-500">يتم ترتيب الدور بحسب وقت وتذكرة الحجز</span>
            </div>
            ...
            {waitingPatients.length === 0 ? (
              <div className="text-center py-12 space-y-3">
                ...
                <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200">لا يوجد مرضى في قاعة الانتظار حالياً</h4>
                ...
                {waitingPatients.map((booking, idx) => (
                  ...
                  >
                    <div className="flex items-start sm:items-center gap-3.5">
                      <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-[#0070cd] dark:text-blue-400 font-mono font-black text-base flex items-center justify-center shrink-0 shadow-xs">
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-auto">
                      <button
                        ...
                        className="bg-[#0070cd] hover:bg-[#005bb0] active:scale-98 text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
                      >
      ...
      {activeTab === 'patients' && (
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-xs">
          <div className="flex items-center justify-between mb-6 pb-3 border-b border-slate-100 dark:border-slate-800">
            <h3 className="font-black text-base text-slate-900 dark:text-white">سجل المرضى والكشوفات المكتملة</h3>
            <span className="text-xs text-slate-500 font-bold">إجمالي الكشوفات: {completedPatients.length}</span>
          </div>
          ...
          {completedPatients.length === 0 ? (
            ...
            <div className="space-y-4">
              {completedPatients.map((booking) => (
                <div 
                  key={booking.id}
                  className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-700"
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-bold text-slate-900 dark:text-white text-sm">{booking.patientName}</h4>
                        <span className="text-xs bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 font-bold px-2 py-0.5 rounded">
                          تم الكشف
                        </span>
                      </div>
                  ...
                  {booking.ePrescription && (
                    <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-3 rounded-xl">
                      <div className="text-xs font-bold text-[#0070cd] dark:text-blue-400 mb-1">
                        التشخيص الطبي: {booking.ePrescription.diagnosis}
                      </div>
                      {booking.ePrescription.medications.length > 0 && (
                        <div className="space-y-1 mt-2">
                          <span className="text-[11px] font-bold text-slate-600 dark:text-slate-400 block">الأدوية الموصوفة:</span>
                          {booking.ePrescription.medications.map((m, i) => (
                            <div key={i} className="text-xs text-slate-700 dark:text-slate-300 font-mono bg-slate-50 dark:bg-slate-800 px-2 py-1 rounded">
      ...
      {activeTab === 'schedule' && (
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-xs max-w-2xl">
          <h3 className="font-black text-base text-slate-900 dark:text-white mb-4">إعدادات مواعيد وساعات عمل العيادة</h3>
          
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">مدة الانتظار التقريبية</label>
              <input
                type="text"
                defaultValue={currentDoctor?.waitingTime || '٢٠ دقيقة'}
                className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm text-slate-900 dark:text-white outline-none focus:border-[#0070cd]"
              />
            </div>

            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">أيام وساعات العمل بالعيادة</label>
              <div className="space-y-2">
                {currentDoctor?.availableDays.map((d, i) => (
                  <div key={i} className="flex items-center justify-between bg-slate-50 dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                    <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{d.dateStr}</span>
            ...
            <div className="pt-3">
              <button
                type="button"
                className="bg-[#0070cd] hover:bg-[#005bb0] text-white px-6 py-2.5 rounded-xl text-xs font-bold transition-all shadow-xs cursor-pointer"
              >
                حفظ التعديلات
              </button>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-xs">
            <div className="flex items-center gap-3 mb-4 pb-3 border-b border-slate-100 dark:border-slate-800">
              <div className="p-2.5 bg-blue-50 dark:bg-slate-800 text-[#0070cd] dark:text-blue-400 rounded-xl">
            ...
            {/* PIN Entry Form */}
            <form onSubmit={handleVerifyPin} className="max-w-md space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 font-mono text-sm font-bold tracking-wider text-slate-900 dark:text-white outline-none focus:border-[#0070cd]"
                  />
                  <button
                    type="submit"
                    disabled={isVerifyingPin}
                    className="bg-[#0070cd] hover:bg-[#005bb0] text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-xs flex items-center gap-1.5 cursor-pointer shrink-0 disabled:opacity-50"
                  >
                    {isVerifyingPin ? (
            ...
            {sharedPatientData && (
              <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800 space-y-4 animate-in fade-in duration-200">
                <div className="bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-4 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="font-black text-sm text-emerald-950 dark:text-emerald-300">
                  ...
                  <span className="text-xs font-bold bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 px-3 py-1 rounded-xl border border-slate-200 dark:border-slate-700">
                    {sharedPatientData.records.length} سجل متاح
                  </span>
                </div>

                {sharedPatientData.records.length === 0 ? (
                  <p className="text-xs text-slate-500 py-4 text-center">لا توجد سجلات طبية مرفوعة ضمن هذا النطاق.</p>
                ) : (
                  <div className="space-y-3">
                    {sharedPatientData.records.map((rec) => (
                      <div key={rec.id} className="bg-slate-50 dark:bg-slate-800/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2">
                        <div className="flex items-center justify-between">
                          <h5 className="font-bold text-sm text-slate-900 dark:text-white">{rec.title}</h5>
                        ...
                        {rec.notes && (
                          <p className="text-xs bg-white dark:bg-slate-900 p-2 rounded-lg text-slate-700 dark:text-slate-300">
                            {rec.notes}
                          </p>
                        )}
                        {rec.medications && rec.medications.length > 0 && (
                          <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-700 flex flex-wrap gap-1.5">
                            {rec.medications.map((m, i) => (
                              <span key={i} className="text-[11px] bg-blue-50 dark:bg-blue-900/40 text-[#0070cd] dark:text-blue-300 px-2 py-0.5 rounded-md font-bold">
                                {m.name} ({m.dose})
                              </span>
                            ))}
                          </div>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-6">
            ...
            <div className="bg-[#002b49] text-white p-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-blue-600/30 flex items-center justify-center text-blue-300">
                  <FileText className="w-5 h-5 text-white" />
                </div>
                <div>
                  <h3 className="font-black text-base">إصدار روشتة إلكترونية فورية</h3>
                  <p className="text-xs text-slate-300">المريض: {activeConsultationBooking.patientName} ({activeConsultationBooking.patientPhone})</p>
                </div>
              </div>
              <button
                onClick={() => setActiveConsultationBooking(null)}
                className="text-slate-300 hover:text-white text-xs bg-white/10 px-3 py-1.5 rounded-xl transition-colors cursor-pointer"
              >
                إغلاق
              </button>
            </div>
            ...
            {/* Modal Form */}
            <form onSubmit={handleCompleteConsultation} className="p-6 space-y-4">
              {isPrescriptionSent ? (
                <div className="text-center py-8 space-y-2">
                  ...
                  <h4 className="text-base font-black text-slate-900 dark:text-white">تم إصدار الروشتة الطبية بنجاح</h4>
                ...
                <>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-xs font-bold text-slate-700 dark:text-slate-300">الأدوية الموصوفة والجرعات</label>
                      <button
                        type="button"
                        onClick={handleAddMedRow}
                        className="text-xs text-[#0070cd] dark:text-blue-400 font-bold flex items-center gap-1 hover:underline cursor-pointer"
                      >
                      ...
                      {medsList.map((med, index) => (
                        <div key={index} className="grid grid-cols-1 sm:grid-cols-12 gap-2 bg-slate-50 dark:bg-slate-800 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 items-center">
                          <input
                            ...
                            className="sm:col-span-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-xs text-slate-800 dark:text-slate-200 outline-none"
                          />
                          <input
                            type="text"
                            ...
                            className="sm:col-span-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-xs text-slate-800 dark:text-slate-200 outline-none"
                          />
                          <input
                            type="text"
                          ...
                          <div className="sm:col-span-2 flex items-center gap-1">
                            <input
                              ...
                              className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-xs text-slate-800 dark:text-slate-200 outline-none"
                            />
                            {medsList.length > 1 && (
                              <button
                                type="button"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-xs text-slate-900 dark:text-white outline-none focus:border-[#0070cd]"
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <button
                      type="submit"
                      className="bg-emerald-600 hover:bg-emerald-700 active:scale-98 text-white px-6 py-2.5 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-md shadow-emerald-500/20 cursor-pointer"
                    >
