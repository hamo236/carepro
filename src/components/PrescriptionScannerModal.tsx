...
export const PrescriptionScannerModal: React.FC<PrescriptionScannerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 dark:bg-black/75 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200 font-['Tajawal',sans-serif]">
      <div 
          ...
          {!selectedImage && !isScanning && !extractedData && (
            <div className="space-y-5">
              ...
              <div 
                ...
                className="border-2 border-dashed border-blue-300 dark:border-slate-700 hover:border-[#0070cd] dark:hover:border-blue-500 bg-blue-50/50 dark:bg-slate-800/40 rounded-3xl p-6 sm:p-8 text-center cursor-pointer transition-all hover:bg-blue-50/80 group"
              >
                <input 
                  type="file" 
                  ref={fileInputRef} 
                  ...
                  className="hidden" 
                />
                <div className="w-14 h-14 bg-blue-100 dark:bg-slate-700 text-[#0070cd] dark:text-blue-400 rounded-2xl flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform">
                ...
                <button
                  type="button"
                  className="bg-[#0070cd] group-hover:bg-[#005bb0] text-white text-xs font-bold px-4 py-2 rounded-xl transition-colors inline-flex items-center gap-1.5"
                >
              ...
              <div>
                <div className="flex items-center gap-2 mb-2 text-xs font-bold text-slate-700 dark:text-slate-300">
                  <FileText className="w-4 h-4 text-[#0070cd]" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {SAMPLE_PRESCRIPTIONS.map((sample) => (
                    <button
                      ...
                      className="p-3.5 text-right bg-slate-50 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-slate-700/60 border border-slate-200 dark:border-slate-700 rounded-2xl transition-all cursor-pointer flex items-center justify-between gap-3 group"
                    >
                      <div className="space-y-0.5">
                        <div className="font-bold text-slate-900 dark:text-white text-xs group-hover:text-[#0070cd] transition-colors">
                          {sample.name}
                        </div>
                      ...
                      <Sparkles className="w-4 h-4 text-[#0070cd] dark:text-blue-400 shrink-0 opacity-80 group-hover:scale-110" />
                    </button>
                  ))}
                </div>
              </div>
          ...
          {isScanning && (
            <div className="py-12 px-4 text-center space-y-4">
              <div className="relative w-24 h-24 mx-auto mb-4">
                <div className="w-24 h-24 rounded-2xl bg-blue-50 dark:bg-slate-800 border border-blue-200 dark:border-slate-700 flex items-center justify-center overflow-hidden">
                  <FileText className="w-12 h-12 text-[#0070cd] dark:text-blue-400" />
                  {/* Scanning Laser Line */}
                  <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-transparent via-blue-500 to-transparent animate-pulse shadow-[0_0_12px_rgba(0,112,205,0.8)]"></div>
                </div>
                <div className="absolute -bottom-2 -right-2 bg-[#0070cd] text-white p-1.5 rounded-full animate-spin">
              ...
              <p className="text-xs text-blue-600 dark:text-blue-400 font-bold max-w-md mx-auto animate-pulse">
                {scanStep}
              </p>
            </div>
          )}
            ...
            <div className="space-y-4 animate-in fade-in duration-300">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/80 rounded-2xl p-4 border border-slate-200 dark:border-slate-700 space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  ...
                  <div>
                    ...
                    <span className="text-[#0070cd] dark:text-blue-400 font-bold">{extractedData.diagnosis}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-1.5 text-xs font-black text-slate-900 dark:text-white">
                  <Pill className="w-4 h-4 text-[#0070cd]" />
                ...
                <div className="space-y-2">
                  {extractedData.medications.map((med, idx) => (
                    ...
                    >
                      ...
                      <span className="text-[11px] bg-blue-50 dark:bg-slate-700 text-[#0070cd] dark:text-blue-400 px-2.5 py-1 rounded-lg font-bold shrink-0 self-start sm:self-center">
                        {med.duration}
                      </span>
                    </div>
                  ))}
        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3">
          ...
          {extractedData && (
            <button
              ...
              className="bg-[#0070cd] hover:bg-[#005bb0] active:scale-95 text-white text-xs font-black px-6 py-2.5 rounded-xl transition-all shadow-sm flex items-center gap-2 cursor-pointer"
            >
              {isSuccessSaved ? (
                <>
