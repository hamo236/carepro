    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-['Tajawal',sans-serif]">
        ...
        <div>
          <div className="text-xs text-slate-500 dark:text-slate-400 mb-1 flex items-center gap-1.5 font-medium">
            ...
            {selectedSpecialty && (
              <>
                <span>/</span>
                <span className="text-[#0066b2] dark:text-blue-400 font-bold">
                  {SPECIALTIES.find(s => s.id === selectedSpecialty)?.name}
                </span>
              </>
            )}
          </div>
          <h1 className="text-2xl font-black text-[#002b49] dark:text-white">
            {selectedSpecialty 
        ...
        <div className="flex flex-wrap items-center gap-3 self-start md:self-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 text-xs font-bold py-2 px-3 rounded-xl outline-none focus:border-[#0066b2] cursor-pointer"
            >
      ...
      <div className="mb-6 overflow-x-auto pb-2 scrollbar-none">
        <div className="flex items-center gap-2 min-w-max">
          <button
            ...
            className={`text-xs font-bold px-4 py-2 rounded-2xl border transition-all duration-200 cursor-pointer ${
              !selectedSpecialty || selectedSpecialty === 'all'
                ? 'bg-[#0066b2] text-white border-[#0066b2] shadow-xs'
                : 'bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800 hover:border-[#0066b2]'
            }`}
          >
            جميع التخصصات
          </button>
      ...
      {viewMode === 'map' ? (
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start animate-in fade-in duration-300">
          ...
          {/* Sidebar Filters */}
          <aside className="w-full lg:w-72 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/90 dark:border-slate-800 p-5 shadow-xs shrink-0 lg:sticky lg:top-24">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800 mb-4">
              <div className="flex items-center gap-1.5 text-slate-800 dark:text-slate-200 font-bold text-sm">
                <SlidersHorizontal className="w-4 h-4 text-[#0066b2] dark:text-blue-400" />
                <span>تصفية الأطباء</span>
              </div>
              <button
                onClick={handleClearAllFilters}
                className="text-xs text-[#0066b2] dark:text-blue-400 hover:underline font-bold flex items-center gap-1 cursor-pointer"
              >
            ...
            <div className="space-y-5 text-xs text-slate-700 dark:text-slate-300">
              ...
              {/* Quick search input */}
              <div>
                <label className="block font-bold text-slate-800 dark:text-slate-200 mb-1.5">ابحث بالاسم أو العيادة</label>
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-2xl py-2 px-2.5 text-xs outline-none focus:border-[#0066b2] focus:bg-white dark:focus:bg-slate-800"
                  />
              ...
              {/* Specialty */}
              <div>
                <label className="block font-bold text-slate-800 dark:text-slate-200 mb-1.5">التخصص الطبي</label>
                <select
                  value={selectedSpecialty}
                  onChange={(e) => setSelectedSpecialty(e.target.value)}
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-2xl py-2 px-2.5 text-xs font-bold outline-none cursor-pointer"
                >
              ...
              {/* City & Area */}
              <div>
                <label className="block font-bold text-slate-800 dark:text-slate-200 mb-1.5">المحافظة والمنطقة</label>
                <div className="space-y-1.5">
                  <select
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-2xl py-2 px-2.5 text-xs outline-none cursor-pointer"
                  >
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-2xl py-2 px-2.5 text-xs outline-none cursor-pointer disabled:opacity-50"
                  >
              ...
              {/* Academic Title */}
              <div>
                <label className="block font-bold text-slate-800 dark:text-slate-200 mb-2">الدرجة العلمية</label>
                <div className="space-y-1.5">
                  {[
                    { id: 'all', label: 'الكل' },
              ...
              {/* Gender */}
              <div>
                <label className="block font-bold text-slate-800 dark:text-slate-200 mb-2">نوع الطبيب</label>
          ...
          <main className="flex-1 w-full space-y-4">
            <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 bg-white dark:bg-slate-900 p-3 rounded-2xl border border-slate-200 dark:border-slate-800">
              <span>
                {isLoading ? (
                  <span className="font-bold text-[#0066b2] dark:text-blue-400">جاري تحميل قائمة الأطباء والمواعيد المتاحة...</span>
                ) : sorted.length > 0 ? (
